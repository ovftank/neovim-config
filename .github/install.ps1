...
$enableAnimation = Read-Host "enable animation? (y/n)"
$branch = if ($enableAnimation -eq "y" -or $enableAnimation -eq "Y") { "main" } else { "dev" }

Write-Host "cloning branch: $branch" -ForegroundColor Cyan
try {
    git clone --depth=1 --filter=blob:none --branch=$branch https://github.com/ovftank/neovim-config.git $nvimPath
