...
keymap.set("i", "<C-v>", '<C-o>"+p', { desc = "paste from clipboard" })
...
keymap.set("i", "<C-z>", "<Cmd>undo<CR>", { desc = "undo" })
...
-- FAST PASTE MODE
-- =============================================================================
local function toggle_fast_paste()
    local current_syntax = vim.bo.syntax
    local current_foldexpr = vim.wo.foldexpr

    if current_syntax ~= "" then
        vim.bo.syntax = ""
        vim.wo.foldexpr = ""
        vim.notify("fast paste mode: on (syntax & fold disabled)", vim.log.levels.INFO)
    else
        vim.bo.syntax = current_syntax
        vim.wo.foldexpr = current_foldexpr
        vim.notify("fast paste mode: off", vim.log.levels.INFO)
    end
end

keymap.set("n", "<M-p>", toggle_fast_paste, { desc = "toggle fast paste mode" })
